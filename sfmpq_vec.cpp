	...
	while (ptr1 < vec->nnz && ptr2 < src->nnz) {
		if (vec->indices[ptr1] == src->indices[ptr2]) {
			...
			if (!scalar_is_zero(src->entries + ptr2)) {
				_sparse_vec_set_entry(tmpvec, src->indices[ptr2], src->entries + ptr2);
			}
			ptr2++;
		}
	}
...
int sfmpq_vec_sub_scalar_sorted(sfmpq_vec_t vec, const sfmpq_vec_t src, const fmpq_t a) {
	// -1 : Different lengths
	if (vec->len != src->len)
		return -1;
...
int sfmpq_vec_sub_scalar_sorted_cached(sfmpq_vec_t vec, const sfmpq_vec_t src, sfmpq_vec_t cache, const fmpq_t a) {
	// -1 : Different lengths
	if (vec->len != src->len)
		return -1;
