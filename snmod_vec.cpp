...
void snmod_vec_rescale(snmod_vec_t vec, ulong scalar, nmod_t p) {
	_nmod_vec_scalar_mul_nmod_shoup(vec->entries,vec->entries,vec->nnz,scalar,p);
}
		...
		for (size_t i = 0; i < src->nnz; i++) {
			...
			if (entry != NULL) {
				*entry = nmod_add(*entry, src->entries[i], p);
				...
				vec->nnz++;
				// SET_BIT_ONE(vec->bitlist, src->indices[i] / src->lenstep);
			}
		}
	}
...
int snmod_vec_add_densed(snmod_vec_t vec, ulong* src, nmod_t p) {
	for (size_t i = 0; i < vec->nnz; i++) {
		src[vec->indices[i]] = nmod_add(src[vec->indices[i]], vec->entries[i], p);
	}
	vec->nnz = 0;
...
int snmod_vec_sub_scalar(snmod_vec_t vec, const snmod_vec_t src, const ulong a, nmod_t p) {
	...
	if (vec->nnz < 100) {
		for (size_t i = 0; i < src->nnz; i++) {
			ulong* entry = sparse_vec_entry(vec, src->indices[i]);
			if (entry != NULL) {
				*entry = nmod_sub(*entry, n_mulmod_shoup(a, src->entries[i], a_pr, p.n), p);
			}
			else {
				vec->indices[vec->nnz] = src->indices[i];
				vec->entries[vec->nnz] = n_mulmod_shoup(a, nmod_neg(src->entries[i], p), a_pr, p.n);
				vec->nnz++;
		...
		slong oldvecnnz = vec->nnz; // vec->nnz will change in the following loop
		...
		while (ptr1 < oldvecnnz && ptr2 < src->nnz) {
			if (indices[ptr1] == src->indices[ptr2]) {
				...
				*entry = nmod_sub(*entry, n_mulmod_shoup(a, src->entries[ptr2], a_pr, p.n), p);
				ptr1++;
				ptr2++;
			}
				...
				vec->entries[vec->nnz] = n_mulmod_shoup(a, nmod_neg(src->entries[ptr2], p), a_pr, p.n);
				vec->nnz++;
				ptr2++;
...
int snmod_vec_sub_scalar_sorted(snmod_vec_t vec, const snmod_vec_t src, const ulong a, nmod_t p) {
	...
	while (ptr1 < vec->nnz && ptr2 < src->nnz) {
		if (vec->indices[ptr1] == src->indices[ptr2]) {
			ulong entry = nmod_add(vec->entries[ptr1], n_mulmod_shoup(na, src->entries[ptr2], na_pr, p.n), p);
			if (entry != 0)
...
int snmod_vec_sub_scalar_sorted_cached(snmod_vec_t vec, const snmod_vec_t src, snmod_vec_t cache, const ulong a, nmod_t p) {
	...
	while (ptr1 < vec->nnz && ptr2 < src->nnz) {
		if (vec->indices[ptr1] == src->indices[ptr2]) {
			...
			if (vec->entries[ptr1] != 0)
				_sparse_vec_set_entry(cache, vec->indices[ptr1], vec->entries[ptr1]);
			ptr1++;
		}
		else {
			if (src->entries[ptr2] != 0) {
