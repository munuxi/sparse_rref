...
template<typename T>
static inline
void _sparse_vec_init(sparse_vec_t<T> vec, ulong len, ulong alloc) {
	vec->len = len;
	vec->nnz = 0;
...
template<typename T>
static inline
void sparse_vec_init(sparse_vec_t<T> vec, ulong len) {
...
template<typename T>
static inline
void sparse_vec_clear(sparse_vec_t<T> vec) {
	free(vec->indices);	
	if constexpr (std::is_same_v<T, fmpq>) {
		for (auto i = 0; i < vec->alloc; i++)
...
template<typename T>
static inline
T* sparse_vec_entry(sparse_vec_t<T> vec, slong index, bool isbinary = false) {
...
template<typename T>
static inline
bool sparse_vec_is_same(const sparse_vec_t<T> vec, const sparse_vec_t<T> src) {
...
template<typename T>
static inline
void sparse_vec_set(sparse_vec_t<T> vec, const sparse_vec_t<T> src) {
...
template<typename T>
static inline
void sparse_vec_move(sparse_vec_t<T> vec, const sparse_vec_t<T> src) {
...
template<typename T>
static inline
void sparse_vec_swap(sparse_vec_t<T> vec, sparse_vec_t<T> src) {
...
static inline
void _sparse_vec_set_entry(sparse_vec_t<T> vec, slong index, S val) {
...
template<typename T, typename S>
static inline
void sparse_vec_set_entry(sparse_vec_t<T> vec, slong index, S val, bool isbinary = false) {
	if (index < 0 || (ulong)index >= vec->len)
		return;
...
//	_nmod_vec_scalar_mul_nmod_shoup(vec->entries, vec->entries, vec->nnz, a, p);
//}
...
template <typename T>
void print_vec_info(sparse_vec_t<T> vec) {
	std::cout << "-------------------" << std::endl;
