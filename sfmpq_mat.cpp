...
static inline void sfmpq_mat_xmay_cached(sfmpq_mat_t mat, slong x, slong y, sfmpq_vec_t cache, fmpq_t a) {
...
static ulong eliminate_row_with_one_nnz(sfmpq_mat_t mat, sparse_mat_t<fmpq*> tranmat, slong* donelist) {
	auto localcounter = 0;
...
static ulong eliminate_row_with_one_nnz_rec(sfmpq_mat_t mat, sparse_mat_t<fmpq*> tranmat, slong* donelist, bool verbose = false) {
	slong depth = 0;
	ulong oldnnz = 0;
	auto localcounter = 0;
	ulong count = 0;
...
static inline
int pair_irr(std::pair<slong, slong> v, std::pair<slong, slong> w, sfmpq_mat_t mat) {
...
auto findmanypivots(sfmpq_mat_t mat, sparse_mat_t<fmpq*> tranmat, slong* rowpivs, std::vector<slong>& colperm,
...
slong findrowpivot(sfmpq_mat_t mat, slong col, slong* rowpivs, std::vector<slong>& colparts, 
	std::vector<slong>& kkparts, std::vector<std::vector<slong>>& rowparts, slong* dolist, fmpq** entrylist, slong& dolist_len, BS::thread_pool& pool) {
		...
		for (auto kk = 0; kk < ps.size(); kk++) {
			...
			pool.detach_loop<slong>(
				0, thecol->nnz, [&](slong i) {
					...
					// sfmpq_mat_xmay_cached(mat, thecol->indices[i], row, cachemat->rows + this_id, entry);
			...
			if (verbose && (countone + kk + 1) % printstep == 0) {
				...
				std::cout <<
					...
					"rank: " << rank << "  " <<
					"nnz: " << now_nnz << "  " <<
					"density: " << (double)100 * now_nnz / (mat->nrow * mat->ncol) << "%  " <<
	...
	for (slong kk = countone; kk < mat->ncol; kk++) {
		...
		pool.detach_loop<slong>(
			0, dolist_len, [&](slong i) {
				...
				// auto this_id = BS::this_thread::get_index().value();
				// sfmpq_mat_xmay_cached(mat, dolist[i], rowi, cachemat->rows + this_id, entrylist[i]);
			}
		);
		pool.wait();
		...
		if (thecol->nnz != 1) {
			pool.detach_loop<slong>(0, thecol->nnz, [&](slong j) {
				if (thecol->indices[j] == p.first)
					return;
				auto entry = sparse_mat_entry(mat, thecol->indices[j], p.second, true);
				// auto this_id = BS::this_thread::get_index().value();
				// sfmpq_mat_xmay_cached(mat, thecol->indices[j], p.first, cachemat->rows + this_id, entry);
