...
// Row x - a*Row y
static inline void snmod_mat_xmay(snmod_mat_t mat, slong x, slong y, ulong a, nmod_t p) {
	snmod_vec_sub_scalar_sorted(mat->rows + x, mat->rows + y, a, p);
}

// Row x - a*Row y
static inline void snmod_mat_xmay_cached(snmod_mat_t mat, slong x, slong y, snmod_vec_t cache, ulong a, nmod_t p) {
	snmod_vec_sub_scalar_sorted_cached(mat->rows + x, mat->rows + y, cache, a, p);
}
...
static void snmod_mat_transpose_pointer(sparse_mat_t<ulong*> mat2, snmod_mat_t mat) {
	for (size_t i = 0; i < mat2->nrow; i++)
		mat2->rows[i].nnz = 0;
...
static ulong eliminate_row_with_one_nnz(snmod_mat_t mat, sparse_mat_t<ulong*> tranmat, slong* donelist) {
	auto localcounter = 0;
...
static ulong eliminate_row_with_one_nnz_rec(snmod_mat_t mat, sparse_mat_t<ulong*> tranmat, slong* donelist, bool verbose = false) {
	slong depth = 0;
	ulong oldnnz = 0;
	...
	do {
		...
		if (verbose) {
			std::cout << "\r-- eliminating rows with only one element, depth: " << depth <<
				", eliminated row: " << count << std::flush;
		}
		count += localcounter;
		depth++;
...
static inline
bool is_irr_tri(slong i, slong j, snmod_mat_t mat, slong* rowpivs) {
	if (i == j)
		return false;
	if (i > j)
...
static inline
bool is_irr(slong v, slong w, snmod_mat_t mat, slong* rowpivs) {
	if (v == w)
		return false;
...
std::vector<slong> findirrpivs_tri(slong start, snmod_mat_t mat, slong* rowpivs, std::vector<bool>& statuslist, int max_depth = INT_MAX) {
...
slong findrowpivot(snmod_mat_t mat, slong col, slong* rowpivs, std::vector<slong>& colparts,
	std::vector<slong>& kkparts, std::vector<std::vector<slong>>& rowparts, slong* dolist, ulong** entrylist, slong& dolist_len, BS::thread_pool& pool) {
	...
	for (size_t i = 0; i < kkparts[mm]; i++) {
		...
		if (rowpivs[row] != -1) // do not choose rows that have been used to eliminate
			continue;
...
auto findmanypivots(snmod_mat_t mat, sparse_mat_t<ulong*> tranmat, slong* rowpivs, std::vector<slong>& colperm,
...
slong* snmod_mat_rref(snmod_mat_t mat, nmod_t p, BS::thread_pool& pool, rref_option_t opt) {
	...
	ulong count = eliminate_row_with_one_nnz_rec(mat, tranmat, rowpivs, verbose);
	rank += count;
	now_nnz = sparse_mat_nnz(mat);
	if (verbose) {
		std::cout << "\n** eliminated " << count << " rows, and reduce nnz: "
			<< init_nnz << " -> " << now_nnz << std::endl;
	}

	snmod_mat_transpose_pointer(tranmat, mat);

	// sort pivots by nnz, it will be faster
	std::stable_sort(colperm.begin(), colperm.end(), [&tranmat](slong a, slong b) {
	...
	for (size_t i = 0; i < mat->ncol; i++) {
		if (tranmat->rows[colperm[i]].nnz == 1) {
			...
			dolist[countone] = row;
			entrylist[countone] = sparse_vec_entry(mat->rows + row, rowpivs[row], true);
			countone++;
			rank++;
		}
	...
	pool.detach_loop<slong>(0, countone,
		[&](slong i) {
			ulong scalar;
	...
	// 	auto therow = mat->rows + i;
	// 	for (size_t j = 0; j < therow->nnz; j++){
	// 			matcher.addEdge(i + 1, therow->indices[j] + 1 + mat->nrow);
	// }
	//
	...
	//     std::cout << "Row " << pair.first << " is matched with Column " << pair.second << std::endl;
	// }
	...
	if (verbose) {
		std::cout << "** found " << connected_components.size()
			<< " components" << std::endl;
	}
	...
	std::stable_sort(colperm.begin() + countone, colperm.end(), [&colparts](slong a, slong b) {
		return colparts[a] < colparts[b];
		});
	...
	while (countone < mat->ncol) {
		...
		auto ps = findmanypivots(mat, tranmat, rowpivs, colperm, colperm.begin() + countone, opt->search_depth);
		...
		for (auto kk = 0; kk < ps.size(); kk++) {
			...
			auto thecol = tranmat->rows + col;
			pool.detach_loop<slong>(
				0, thecol->nnz, [&](slong i) {
					if (thecol->indices[i] == row)
						return;
					auto entry = sparse_mat_entry(mat, thecol->indices[i], col, true);
					// auto this_id = BS::this_thread::get_index().value();
					// snmod_mat_xmay_cached(mat, thecol->indices[i], row, cachemat->rows + this_id, *entry, p);
			...
			if (verbose && (countone + kk + 1) % opt->printlen == 0) {
				...
				std::cout <<
					"\r-- Col: " <<
					countone + kk + 1 << "/" << mat->ncol << "  " <<
					"row to eliminate: " << tranmat->rows[*pp.second].nnz - 1 << "  " <<
					"rank: " << rank << "  " <<
					"nnz: " << now_nnz << "  " <<
					"density: " << (double)now_nnz / (mat->nrow * mat->ncol) << "  " <<
					"speed: " << 1 / usedtime(start, end) << " col/s" << std::flush;
			}
		}

		std::vector<slong> tempcolperm(colperm.begin() + countone, colperm.end());
		...
		sparse_mat_transpose_pointer(tranmat, mat);
		// sort pivots by nnz, it will be faster
		std::stable_sort(colperm.begin() + countone, colperm.end(), [&tranmat, &colparts](slong a, slong b) {
	...
	for (auto kk = countone; kk < mat->ncol; kk++) {
		...
		for (auto i = 0; i < mat->nrow; i++)
			dolist[i] = -1;
		slong rowi = findrowpivot(mat, pivot, rowpivs, colparts,
			kkparts, rowparts, dolist, entrylist, dolist_len, pool);
		...
		if (kk % opt->sort_step == 0 || kk == mat->ncol - 1) {
			...
			if (verbose)
				std::cout << std::endl;
			count = eliminate_row_with_one_nnz_rec(mat, tranmat, rowpivs, verbose);
			rank += count;
			now_nnz = sparse_mat_nnz(mat);
			if (verbose) {
				std::cout << "\n** eliminated " << count << " rows, and nnz is "
			...
			// sort pivots by nnz, it will be faster
			std::stable_sort(colperm.begin() + kk + 1, colperm.end(), [&tranmat, &colparts](slong a, slong b) {
				if (colparts[a] < colparts[b]) {
					return true;
				}
				else if (colparts[a] == colparts[b]) {
			...
			//		"alloc " << oldalloc << " shrink to 2*nnz " << std::endl;
			//}
		}
		...
		if (thecol->nnz != 1) {
			pool.detach_loop<slong>(0, thecol->nnz, [&](slong j) {
				if (thecol->indices[j] == pp.first)
					return;
				auto entry = sparse_mat_entry(mat, thecol->indices[j], pp.second, true);
		...
		if (verbose) {
			if (i % opt->printlen == 0 || i == pivots.size() - 1) {
				...
				std::cout <<
					...
					"nnz: " << now_nnz << "  " <<
					"density: " << (double)now_nnz / (mat->nrow * mat->ncol) << "  " <<
					"speed: " << 1 / usedtime(start, end) << " row/s" << std::flush;
			}
		}
	}
